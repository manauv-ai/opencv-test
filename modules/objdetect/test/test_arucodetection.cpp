...
/**
 * @brief Check that a cell ratio encoded dictionary is equivalent to its binary dict
 * (e.g. DICT_4X4_50_RATIO and DICT_4X4_50)
 *
...
static void runArucoDetectionCellRatioDict(ArucoAlgParams arucoAlgParam,
                                           ...
                                           aruco::PredefinedDictionaryType ratioDictType) {
    SCOPED_TRACE(cv::format("binary dict: %d ratio dict: %d", (int)binaryDictType, (int)ratioDictType));
    const bool detectInvertedMarker = (arucoAlgParam == ArucoAlgParams::DETECT_INVERTED_MARKER);

    aruco::Dictionary binaryDict = aruco::getPredefinedDictionary(binaryDictType);
    aruco::Dictionary ratioDict = aruco::getPredefinedDictionary(ratioDictType);
...
// binary dictionaries and their cell ratio equivalent
static const std::pair<aruco::PredefinedDictionaryType, aruco::PredefinedDictionaryType> cellRatioDictPairs[] = {
    {aruco::DICT_4X4_50, aruco::DICT_4X4_50_RATIO},
    {aruco::DICT_5X5_50, aruco::DICT_5X5_50_RATIO},
    {aruco::DICT_6X6_50, aruco::DICT_6X6_50_RATIO},
    {aruco::DICT_7X7_50, aruco::DICT_7X7_50_RATIO},
};

TEST(CV_ArucoDetectionCellRatioDict, algorithmic) {
    for (const auto& dictPair : cellRatioDictPairs)
        runArucoDetectionCellRatioDict(ArucoAlgParams::USE_DEFAULT, dictPair.first, dictPair.second);
}

TEST(CV_InvertedArucoDetectionCellRatioDict, algorithmic) {
    for (const auto& dictPair : cellRatioDictPairs)
        runArucoDetectionCellRatioDict(ArucoAlgParams::DETECT_INVERTED_MARKER, dictPair.first, dictPair.second);
}
