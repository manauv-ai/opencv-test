...
struct CellRatioDistance {
    CellRatioDistance(const Mat &onlyCellPixelRatio, int markerSize, float validBitIdThreshold)
        : totalCells(markerSize * markerSize),
          bounds(2 * totalCells),
          lower(bounds.data()), upper(bounds.data() + totalCells) {
        uint8_t* lowerWritable = bounds.data();
        uint8_t* upperWritable = lowerWritable + totalCells;
        ...
        for(int j = 0; j < markerSize; j++) {
            ...
            for(int i = 0; i < markerSize; i++, cell++) {
                ...
                lowerWritable[cell] = saturate_cast<uint8_t>(std::ceil(observed - threshold));
                upperWritable[cell] = saturate_cast<uint8_t>(std::floor(observed + threshold));
            }
        }
    }
    ...
    int distanceToId(const Mat& bytesList, int id, bool allRotations, int& rotation) const {
        ...
        for(unsigned int r = 0; r < nRotations; r++, expectedRatios += totalCells) {
            ...
            for(int i = 0; i < totalCells; i++)
                currentDistance += (expectedRatios[i] < lower[i]) | (expectedRatios[i] > upper[i]);
    ...
    const int totalCells;
    std::vector<uint8_t> bounds;   // valid [lower | upper] expected ratio of each cell, in percent
...
Dictionary getPredefinedDictionary(PredefinedDictionaryType name) {
    ...
    static const Dictionary DICT_4X4_50_RATIO_DATA = Dictionary(Mat(50, 4 * 4, CV_8UC4, (uchar*)DICT_4X4_50_RATIO_CELLS), 4, (4-1)/2, DICT_ENCODING_CELL_RATIO);
    static const Dictionary DICT_5X5_50_RATIO_DATA = Dictionary(Mat(50, 5 * 5, CV_8UC4, (uchar*)DICT_5X5_50_RATIO_CELLS), 5, (8-1)/2, DICT_ENCODING_CELL_RATIO);
    static const Dictionary DICT_6X6_50_RATIO_DATA = Dictionary(Mat(50, 6 * 6, CV_8UC4, (uchar*)DICT_6X6_50_RATIO_CELLS), 6, (13-1)/2, DICT_ENCODING_CELL_RATIO);
    static const Dictionary DICT_7X7_50_RATIO_DATA = Dictionary(Mat(50, 7 * 7, CV_8UC4, (uchar*)DICT_7X7_50_RATIO_CELLS), 7, (19-1)/2, DICT_ENCODING_CELL_RATIO);
    ...
    case DICT_4X4_50_RATIO:
        return Dictionary(DICT_4X4_50_RATIO_DATA);
    case DICT_5X5_50_RATIO:
        return Dictionary(DICT_5X5_50_RATIO_DATA);
    case DICT_6X6_50_RATIO:
        return Dictionary(DICT_6X6_50_RATIO_DATA);
    case DICT_7X7_50_RATIO:
        return Dictionary(DICT_7X7_50_RATIO_DATA);
    }
    return Dictionary(DICT_4X4_50_DATA);
}
